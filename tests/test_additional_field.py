...
def test_add_new_additional_field_success(
    client, new_user, new_record, new_additional_field
):
    """
...
def test_add_new_additional_field_fail_1(
    client, new_user, new_record, new_additional_field
):
    """
    ...
    expected_response_message = (
        f"Record with id {new_additional_field['record_id']} doesn't exist"
    )
...
def test_add_new_additional_field_fail_2(
    client, new_user, new_record, new_additional_field
):
    """
    ...
    expected_response_message = (
        f"Record {new_record['id']} doesn't belong to the current user {new_user['id']}"
    )
...
def test_add_new_additional_field_fail_3(
    client, new_user, new_record, new_additional_field
):
    """
    ...
    expected_response_message = (
        f"Additional field with id {new_additional_field['id']} already exists"
    )
...
def test_delete_additional_field_success(
    client, new_user, new_record, new_additional_field
):
    """
    ...
    response = client.delete(
        "/additional_fields",
        headers={"x-access-token": f"{token}"},
        query_string={"id": new_additional_field["id"]},
    )

    expected_response_message = (
        f"Additional field {new_additional_field['id']} deleted successfully"
    )
...
def test_delete_additional_field_fail_1(client, new_user):
    ...
    response = client.delete(
        "/additional_fields", headers={"x-access-token": f"{token}"}
    )

    assert response.status_code == 400
    assert (
        b"Additional field id is missing in URI arguments" in response.data
        and b"error" in response.data
    )
...
def test_delete_additional_field_fail_2(client, new_user, new_additional_field):
    ...
    expected_response_message = (
        f"Additional field with id {new_additional_field['id']} doesn't exist"
    )
    assert response.status_code == 404
    assert (
        expected_response_message.encode() in response.data
...
def test_delete_additional_field_fail_3(
    client, new_user, new_record, new_additional_field
):
    """
...
def test_get_additional_fields_success(
    client, new_user, new_record, new_additional_field
):
    """
    ...
    assert (
        db.session.query(models.AdditionalField).get(new_additional_field_2["id"])
        != None
    )
    ...
    response = client.get(
        "/additional_fields",
        headers={"x-access-token": f"{token}"},
        query_string={"id": f"{new_record['id']}"},
    )
    assert response.status_code == 200
    import json
...
def test_get_additional_fields_fail_1(client, new_user):
    ...
    assert response.status_code == 400
    assert (
        b"Record id is missing in URI arguments" in response.data
        and b"error" in response.data
    )
...
def test_get_additional_fields_fail_4(
    client, new_user, new_record, new_additional_field
):
    """
...
def test_edit_additional_field_success(
    client, new_user, new_record, new_additional_field
):
    """
    ...
    response = client.put(
        "/additional_fields",
        headers={"x-access-token": f"{token}"},
        json=edited_additional_field,
    )
    expected_response_message = (
        f"Additional field {new_additional_field['id']} changed successfully"
    )
    assert response.status_code == 200
    assert (
        expected_response_message.encode() in response.data
...
def test_edit_additional_field_fail_1(
    client, new_user, new_record, new_additional_field
):
    """
...
def test_edit_additional_field_fail_3(
    client, new_user, new_record, new_additional_field
):
    """
