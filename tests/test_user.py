...
def test_signup_new_user_success(client, new_user: dict):
    ...
    expected_response_message = f"User created successfully {new_user[nameof(models.User.id)]}"
...
def test_signup_new_user_fail_1(client, new_user: dict):
    ...
    expected_response_message = f"User with id {new_user[nameof(models.User.id)]} already exists"
