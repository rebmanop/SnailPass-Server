...
def test_add_new_record_fail_1(client, new_user, new_record):
    ...
    expected_response_message = (
        f"Record with id {new_record[nameof(models.Record.id)]} already exists"
    )
...
def test_delete_record_success(client, new_user, new_record):
    ...
    expected_response_message = (
        f"Record {new_record[nameof(models.Record.id)]} deleted successfully"
    )
...
def test_edit_record_success(client, new_user, new_record):
    ...
    response = client.put(
        "/records", headers={"x-access-token": f"{token}"}, json=edited_record
    )
    expected_respones_message = f"Record {new_record['id']} changed successfully"
    assert response.status_code == 200
    assert (
        expected_respones_message.encode() in response.data
        and b"success" in response.data
    )
    assert (
        db.session.query(models.Record).get(new_record["id"]).password
        == edited_record["password"]
    )
