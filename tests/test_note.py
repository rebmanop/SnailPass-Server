...
def test_add_new_note_success(client, new_user, new_note):
    ...
    response = client.post(
        "/notes", headers={"x-access-token": f"{token}"}, json=new_note
    )
...
def test_delete_note_fail_3(client, new_user, new_note):
    ...
    expected_response_message = (
        f"Note {new_note['id']} doesn't belong to the current user {new_user['id']}"
    )
...
def test_get_note_fail(client, new_user):
    ...
    assert response.status_code == 404
    assert (
        expected_resoponse_message.encode() in response.data
        and b"error" in response.data
    )
...
def test_edit_note_success(client, new_user, new_note):
    ...
    response = client.put(
        "/notes", headers={"x-access-token": f"{token}"}, json=edited_note
    )
    ...
    assert (
        db.session.query(models.Note).get(new_note["id"]).content
        == edited_note["content"]
    )
