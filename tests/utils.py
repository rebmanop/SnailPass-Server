...
def add_new_user_to_mock_db(new_user: dict) -> None:
    ...
    """
    additionaly_hashed_mp = hash_mp_additionally(password_hash=new_user[nameof(models.User.master_password_hash)], 
                                                salt=new_user[nameof(models.User.email)])

    
    new_user_model = models.User(id=new_user[nameof(models.User.id)], 
...
def add_new_record_to_mock_db(new_user: dict, new_record: dict) -> None:
    ...
    new_record_model = models.Record(id=new_record[nameof(models.Record.id)], 
...
def add_new_note_to_mock_db(new_user: dict, new_note: dict) -> None:
    ...
    new_note_model = models.Note(id=new_note[nameof(models.Note.id)], 
...
def add_new_additional_field_to_mock_db(new_record: dict, new_additional_field: dict) -> None:
    """
    Adds new_note to mock db for tests
    """
    
    new_additional_field_model = models.AdditionalField(id=new_additional_field[nameof(models.AdditionalField.id)], 
...
def get_mock_token(new_user: dict, token_ttl_minutes = api.TOKEN_TTL):
    ...
    """
    data = {'id': new_user[nameof(models.User.id)], 'exp': datetime.datetime.utcnow() + datetime.timedelta(minutes=token_ttl_minutes)}
