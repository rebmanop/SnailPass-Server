...
class Note(Resource):
    ...
    def post(self, current_user):
        ...
        if db.session.query(models.Note).get(args[nameof(models.Note.id)]):
            raise err.APIResourceAlreadyExistsError(
                f"Note with id {args[nameof(models.Note.id)]} already exists"
            )
    ...
    def put(self, current_user):
        ...
        if not note:
            raise err.APIResourceNotFoundError(
                f"Note with id {args[nameof(models.Note.id)]} doesn't exist"
            )
    ...
    def get(self, current_user):
        ...
        if len(current_user.notes) == 0:
            raise err.APIResourceNotFoundError(
                f"Current user {current_user.id} has no notes"
            )
        else:
            return [
                marshal(note, NOTE_RESOURCE_FIELDS) for note in current_user.notes
            ], 200
